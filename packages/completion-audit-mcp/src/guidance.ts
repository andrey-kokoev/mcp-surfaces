...
const SURFACE_ID = "completion-audit";
const GUIDANCE_TOOL = "completion_audit_guidance";
const PURPOSE = "Durable completion requirement, evidence, and verdict recording.";
...
export function buildGuidanceResult(args: GuidanceRecord = {}): GuidanceRecord {
  ...
  return {
    ...
    examples: [
      { intent: 'First use', call: 'completion_audit_guidance({})' },
      { intent: 'Tool-specific help', call: "completion_audit_guidance({ tool: \"<tool_name>\" })" },
      { intent: 'Workflow-specific help', call: "completion_audit_guidance({ workflow: \"<workflow_name>\" })" }
    ],
    anti_patterns: [
