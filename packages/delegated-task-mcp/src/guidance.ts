...
const SURFACE_ID = "delegated-task";
const GUIDANCE_TOOL = "delegated_task_guidance";
const PURPOSE = "Durable delegated task orchestration with acceptance contracts and handoffs.";
...
export function buildGuidanceResult(args: GuidanceRecord = {}): GuidanceRecord {
  ...
  return {
    ...
    examples: [
      { intent: 'First use', call: 'delegated_task_guidance({})' },
      { intent: 'Tool-specific help', call: "delegated_task_guidance({ tool: \"<tool_name>\" })" },
      { intent: 'Workflow-specific help', call: "delegated_task_guidance({ workflow: \"<workflow_name>\" })" }
    ],
    anti_patterns: [
