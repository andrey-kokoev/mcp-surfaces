...
const GUIDANCE_TOOL = "launcher_guidance";
const PURPOSE = "Read-only launcher registry, option matrix, plan, and coherence inspection.";
...
export function buildGuidanceResult(args: GuidanceRecord = {}): GuidanceRecord {
  ...
  return {
    ...
    examples: [
      { intent: 'First use', call: 'launcher_guidance({})' },
      { intent: 'Tool-specific help', call: "launcher_guidance({ tool: \"<tool_name>\" })" },
      { intent: 'Workflow-specific help', call: "launcher_guidance({ workflow: \"<workflow_name>\" })" }
    ],
    anti_patterns: [
