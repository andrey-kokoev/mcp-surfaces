...
export function buildGuidanceResult(args: GuidanceRecord = {}): GuidanceRecord {
  ...
  return {
    ...
    examples: [
      { intent: 'First use', call: 'mailbox_guidance({})' },
      { intent: 'Tool-specific help', call: "mailbox_guidance({ tool: \"<tool_name>\" })" },
      { intent: 'Workflow-specific help', call: "mailbox_guidance({ workflow: \"<workflow_name>\" })" }
    ],
    anti_patterns: [
