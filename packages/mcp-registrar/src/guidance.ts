...
const PURPOSE = "MCP surface registrar for site and carrier surface binding.";
...
export function buildGuidanceResult(args: GuidanceRecord = {}): GuidanceRecord {
  ...
  return {
    ...
    examples: [
      ...
      { intent: 'Tool-specific help', call: "registrar_guidance({ tool: \"<tool_name>\" })" },
      { intent: 'Workflow-specific help', call: "registrar_guidance({ workflow: \"<workflow_name>\" })" }
    ],
    anti_patterns: [
