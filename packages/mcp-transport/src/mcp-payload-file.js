...
export function resolveToolPayloadArgs({ siteRoot, toolName, args, allowedTools, maxBytes = DEFAULT_MAX_BYTES, payloadDir = DEFAULT_PAYLOAD_DIR, payloadRefMode = 'replace_args', }) {
    ...
    const payloadRef = typeof input.payload_ref === 'string' && input.payload_ref.trim().length > 0
        ...
        : null;
    if (payloadPath && payloadRef)
        throw new Error('payload_transport_must_choose_one_of_payload_path_or_payload_ref');
    if (!payloadPath && !payloadRef)
    ...
    if (!stat.isFile())
        throw new Error(`payload_path_not_file: ${normalizePath(relative(root, absolutePath))}`);
    if (stat.size > maxBytes)
        throw new Error(`payload_path_too_large: ${stat.size} > ${maxBytes}`);
    let parsed;
    try {
...
export function attachPayloadSource(result, payloadSource) {
    if (!payloadSource || !result || typeof result !== 'object' || Array.isArray(result))
        return result;
    return { ...result, payload_source: payloadSource };
}
export function enforceInlinePayloadLimit({ toolName, args, limit = DEFAULT_INLINE_PAYLOAD_CHAR_LIMIT, exemptFields = DEFAULT_INLINE_PAYLOAD_EXEMPT_FIELDS, objectPayloadFields = DEFAULT_INLINE_OBJECT_PAYLOAD_FIELDS, allowPayloadCreation = false, } = {}) {
    const input = asRecord(args);
    if (allowPayloadCreation && isPayloadWorkspaceTool(toolName))
        return;
    const violations = [];
    ...
    throw new Error(`inline_payload_too_long: field=${first.field} length=${first.length} threshold=${limit} remediation=call mcp_payload_create with {"payload":{...}} then call ${toolName} with {"payload_ref":"mcp_payload:<id>@v1"}`);
}
...
export function buildOutputRefToolContent({ siteRoot, toolName, value, isError = false, limit = DEFAULT_INLINE_OUTPUT_CHAR_LIMIT, createdBy = process.env.NARADA_AGENT_ID || null, } = {}) {
...
function outputCreate({ siteRoot, toolName, value, fullText, inlineLimit, createdBy, maxBytes = DEFAULT_OUTPUT_MAX_BYTES, outputDir = DEFAULT_OUTPUT_DIR }) {
    ...
    if (byteSize > maxBytes)
        throw new Error(`mcp_output_too_large: ${byteSize} > ${maxBytes}`);
...
function readOutputRecord({ siteRoot, ref, maxBytes = DEFAULT_OUTPUT_MAX_BYTES, outputDir = DEFAULT_OUTPUT_DIR }) {
    ...
    if (!stat.isFile())
        throw new Error(`output_ref_not_file: ${ref}`);
    if (stat.size > maxBytes)
        throw new Error(`output_ref_too_large: ${stat.size} > ${maxBytes}`);
    let record;
    try {
    ...
    if (!record || typeof record !== 'object' || Array.isArray(record))
        throw new Error(`output_ref_record_must_be_object: ${ref}`);
    if (record.schema !== 'narada.mcp_output_ref.v1')
        throw new Error(`output_ref_schema_unsupported: ${record.schema}`);
    if (record.ref !== ref || record.output_id !== parsed.outputId)
        throw new Error(`output_ref_metadata_mismatch: ${ref}`);
...
function normalizeOutputShowLimit(value) {
    if (value === undefined || value === null || value === '')
        return DEFAULT_OUTPUT_SHOW_CHAR_LIMIT;
...
function parseOutputRef(ref) {
    ...
    if (!match)
        throw new Error(`output_ref_invalid: ${value}`);
...
function readPayloadRevision({ siteRoot, ref, maxBytes = DEFAULT_MAX_BYTES, payloadDir = DEFAULT_PAYLOAD_DIR }) {
    ...
    if (!stat.isFile())
        throw new Error(`payload_ref_not_file: ${ref}`);
    if (stat.size > maxBytes)
        throw new Error(`payload_ref_too_large: ${stat.size} > ${maxBytes}`);
    let record;
    try {
        record = JSON.parse(readFileSync(path, 'utf8'));
    }
...
function writeRevision({ siteRoot, payloadDir, record, overwrite }) {
    ...
    if (!overwrite && existsSync(path))
        throw new Error(`payload_revision_already_exists: ${record.ref}`);
    mkdirSync(dirname(path), { recursive: true });
...
function buildRevisionRecord({ payloadId, revision, payload, createdAt, createdBy, source, maxBytes }) {
    ...
    if (byteSize > maxBytes)
        throw new Error(`payload_too_large: ${byteSize} > ${maxBytes}`);
    return {
...
function validateRevisionRecord(record, parsed, statSize, maxBytes) {
    if (!record || typeof record !== 'object' || Array.isArray(record))
        throw new Error(`payload_ref_record_must_be_object: ${parsed.ref}`);
    if (record.schema !== 'narada.mcp_payload.revision.v1')
        throw new Error(`payload_ref_schema_unsupported: ${record.schema}`);
    ...
    if (statSize > maxBytes || record.byte_size > maxBytes)
        throw new Error(`payload_ref_too_large: ${statSize} > ${maxBytes}`);
    const payloadJson = stableJson(record.payload);
    if (sha256(payloadJson) !== record.sha256)
        throw new Error(`payload_ref_sha256_mismatch: ${parsed.ref}`);
}
function parsePayloadRef(ref) {
    ...
    if (!match)
        throw new Error(`payload_ref_invalid: ${value}`);
...
function validatePayloadId(value) {
    ...
    if (!match)
        throw new Error(`payload_id_invalid: ${value}`);
    return value.trim();
}
...
//# sourceMappingURL=mcp-payload-file.js.map