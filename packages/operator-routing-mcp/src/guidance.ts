...
const SURFACE_ID = "operator-routing";
const GUIDANCE_TOOL = "operator_routing_guidance";
const PURPOSE = "User Site operator transcript-to-target routing and inbox fallback packaging.";
...
export function buildGuidanceResult(args: GuidanceRecord = {}): GuidanceRecord {
  ...
  return {
    ...
    examples: [
      { intent: 'First use', call: 'operator_routing_guidance({})' },
      { intent: 'Tool-specific help', call: "operator_routing_guidance({ tool: \"<tool_name>\" })" },
      { intent: 'Workflow-specific help', call: "operator_routing_guidance({ workflow: \"<workflow_name>\" })" }
    ],
    anti_patterns: [
