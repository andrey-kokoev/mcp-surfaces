...
const binding = normalizeExecutionBinding({
  workspace_root: 'C:\\workspace\\mcp-surfaces',
  executor_kind: 'delegated_task',
  correlation_key: 'request-1',
});
  assert.equal(binding.workspace_root.toLowerCase(), 'c:\\workspace\\mcp-surfaces');
...
assert.throws(() => normalizeExecutionBinding({ workspace_root: 'C:\\workspace\\mcp-surfaces', executor_kind: 'delegated_task', correlation_key: 'x', unexpected: true }), /unknown_fields/);
