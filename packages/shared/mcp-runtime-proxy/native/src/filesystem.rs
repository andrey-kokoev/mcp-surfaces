...
const DEFAULT_GREP_IGNORES: &[&str] = &[
    ...
    "**/coverage/**", "**/.next/**", "**/.turbo/**", "**/.cache/**",
    "**/.venv/**", "**/__pycache__/**", "**/target/**",
    "**/.ai/runtime/**", "**/.ai/tmp/**", "**/.ai/output/**",
    "**/.narada/runtime/**", "**/.narada/tmp/**", "**/.tmp-tests/**",
];
const GENERATED_MARKERS: &[&str] = &[
    "/.ai/runtime/", "/.ai/tmp/", "/.ai/output/", "/.narada/runtime/",
...
fn parse_state(args: &[String]) -> Result<State, String> {
    ...
    while index < args.len() {
        match args[index].as_str() {
            "--mode" => { index += 1; mode = args.get(index).cloned().ok_or("filesystem_mode_required")?; }
            "--allowed-root" => { index += 1; roots.push(args.get(index).cloned().ok_or("filesystem_allowed_root_required")?); }
            "--anchored-allowed-root" => { index += 1; anchored.push(args.get(index).cloned().ok_or("filesystem_anchored_root_required")?); }
            "--roots-config" => { index += 1; roots_config = Some(args.get(index).cloned().ok_or("filesystem_roots_config_required")?); }
            "--output-root" => { index += 1; output_root = Some(args.get(index).cloned().ok_or("filesystem_output_root_required")?); }
            "--audit-log-dir" => { index += 1; audit_log_dir = Some(args.get(index).cloned().ok_or("filesystem_audit_log_dir_required")?); }
            "--roots-from-trust-config" | "--roots-from-codex-config" => {
                index += 1;
                let path = args.get(index).cloned().ok_or("filesystem_trust_config_required")?;
    ...
    for root in roots {
        ...
        if allowed_roots.iter().any(|value: &PathBuf| normalize_path(value) == key) { continue; }
        entries.push(json!({"root": path.to_string_lossy(), "provenance": {"source": "explicit_flag", "flag": "--allowed-root"}}));
        allowed_roots.push(path);
    }
    if allowed_roots.is_empty() { return Err("filesystem_mcp_requires_at_least_one_allowed_root".to_string()); }
    Ok(State {
        mode,
        allowed_roots,
        root_entries: entries,
        output_root: absolute(output_root.map(PathBuf::from).unwrap_or_else(|| env::current_dir().unwrap_or_else(|_| PathBuf::from(".")))),
...
fn parse_roots_config(path: &Path) -> Vec<String> {
    ...
    value.get("allowed_roots").and_then(Value::as_array).map(|items| items.iter().filter_map(Value::as_str).map(str::to_string).collect()).unwrap_or_default()
}
...
fn parse_trust_config(path: &Path) -> Vec<String> {
    ...
    for raw in text.lines() {
        let line = raw.trim();
        if let Some(value) = line.strip_prefix("[projects.'").and_then(|value| value.strip_suffix("']")) {
            current = Some(value.to_string());
        } else if line.starts_with('[') { current = None; }
        else if line.eq_ignore_ascii_case("trust_level = \"trusted\"") {
            if let Some(value) = current.clone() { roots.push(value); }
        }
    }
    roots
}
...
fn resolve_anchor(spec: &str) -> Result<String, String> {
    let Some((anchor, relative)) = spec.split_once(':') else { return Err(format!("anchored_allowed_root_requires_anchor:{spec}")); };
    if relative.is_empty() || Path::new(relative).is_absolute() { return Err(format!("anchored_allowed_root_path_must_be_relative:{spec}")); }
    let base = match anchor {
        "user_home" => env::var("USERPROFILE").or_else(|_| env::var("HOME")).map(PathBuf::from).map_err(|_| "user_home_anchor_unavailable".to_string())?,
...
fn handle_request(state: &mut State, request: &Value) -> Option<Value> {
    ...
    let result = match method {
        ...
        "prompts/list" => Ok(json!({"prompts": [{"name": "local_filesystem_tool_usage", "title": "Local Filesystem Tool Usage", "description": format!("Guidance for using local-filesystem-{} tools safely.", state.mode), "arguments": []}]})),
        ...
        _ => Err(FsError::new("unsupported_mcp_method", format!("unsupported_mcp_method: {method}"), json!({"method": method}))),
    };
    Some(match result {
        Ok(value) => json!({"jsonrpc": "2.0", "id": id, "result": value}),
        Err(error) => json!({"jsonrpc": "2.0", "id": id, "error": {"code": -32000, "message": error.message, "data": diagnostic(&error)}}),
    })
}
...
fn prompt_get(state: &State, params: &Value) -> Result<Value, FsError> {
    ...
    if name != "local_filesystem_tool_usage" { return Err(FsError::new("unknown_prompt", format!("unknown_prompt: {name}"), json!({"name": name}))); }
    Ok(json!({"description": format!("Guidance for using local-filesystem-{} tools safely.", state.mode), "messages": [{"role": "user", "content": {"type": "text", "text": format!("Use local-filesystem-{} tools only within allowed roots. Prefer read/search tools before mutation and preserve structuredContent as authoritative.", state.mode)}}]}))
}
...
fn add_diagnostic_details(value: Value) -> Value {
    ...
    details.insert("diagnostic_owner".to_string(), json!("local-filesystem-mcp"));
...
fn read_file(state: &State, args: &Value, range: bool) -> Result<Value, FsError> {
    let (path, root) = resolve_allowed(state, args.get("path").and_then(Value::as_str), if range { "fs_read_file_range" } else { "fs_read_file" })?;
    let (offset, limit) = if range {
        ...
        (integer(args, "offset").unwrap_or(1).max(1), integer(args, "limit").unwrap_or(400).max(1).min(1000))
    };
    let timeout = integer(args, "timeout_ms").unwrap_or(READ_TIMEOUT_MS as i64).max(1).min(60_000) as u64;
    let started = std::time::Instant::now();
    let bytes = fs::read(&path).map_err(|error| FsError::new("fs_read_file_failed", format!("fs_read_file_failed: {error}"), path_details(&path, &root)))?;
    if bytes.contains(&0) { return Err(FsError::new("binary_file_not_supported", format!("binary_file_not_supported: {}", path.display()), path_details(&path, &root))); }
    if started.elapsed().as_millis() as u64 > timeout {
        return Err(FsError::new(if range { "fs_read_file_range_timed_out" } else { "fs_read_file_timed_out" }, "filesystem read timed out", json!({"timeout_ms": timeout, "offset": offset, "limit": limit, "path": path, "root": root})));
    }
    ...
    let selected = if start_index < lines.len() { &lines[start_index..end_index] } else { &[] };
    let content = selected.join("\n");
    let complete = end_index >= lines.len();
    let next_offset = if complete { None } else { Some((end_index + 1) as i64) };
    let (total_lines, total_lines_exact, line_window_complete, content_hash) = if complete {
        ...
        let boundary = text.match_indices('\n').nth(end_index).map(|(index, _)| index + 1).unwrap_or(bytes.len());
        let prefix_len = ((boundary + 65_535) / 65_536 * 65_536).min(bytes.len());
        (Value::Null, false, false, sha256_bytes(&bytes[..prefix_len]))
    };
    Ok(json!({
...
fn stat_tool(state: &State, args: &Value) -> Result<Value, FsError> {
    ...
    let metadata = fs::metadata(&path).map_err(|error| FsError::new("fs_stat_failed", format!("fs_stat_failed: {error}"), path_details(&path, &root)))?;
    ...
    if metadata.is_file() {
        if let Ok(bytes) = fs::read(&path) { value.insert("sha256".into(), json!(sha256_bytes(&bytes))); }
    }
    if metadata.is_dir() {
        let (entry_count, tree_entry_count, tree_sha256, truncated) = directory_fingerprint(&path, &path);
...
fn search_tool(state: &mut State, args: &Value, grep: bool) -> Result<Value, FsError> {
    let operation = if grep { "fs_grep_search" } else { "fs_glob_search" };
    let scope_arg = if grep { args.get("path").and_then(Value::as_str).unwrap_or(".") } else { args.get("directory").and_then(Value::as_str).unwrap_or(".") };
    let (scope, _root) = resolve_allowed(state, Some(scope_arg), operation)?;
    let pattern = args.get("pattern").and_then(Value::as_str).ok_or_else(|| FsError::new(if grep { "grep_requires_pattern" } else { "glob_requires_pattern" }, if grep { "grep_requires_pattern" } else { "glob_requires_pattern" }, json!({})))?;
    let offset = integer(args, "offset").unwrap_or(0).max(0) as usize;
    let limit = integer(args, "limit").unwrap_or(if grep { 80 } else { 100 }).max(1).min(500) as usize;
    let cache_policy = args.get("cache_policy").and_then(Value::as_str).unwrap_or("auto");
    if !["auto", "snapshot", "refresh", "bypass"].contains(&cache_policy) {
        return Err(FsError::new("search_cache_policy_unsupported", format!("search_cache_policy_unsupported: {cache_policy}"), json!({"cache_policy": cache_policy})));
    }
    ...
    if grep && !["files_with_matches", "count_matches", "content"].contains(&output_mode) {
        return Err(FsError::new("grep_output_mode_unsupported", format!("grep_output_mode_unsupported: {output_mode}"), json!({"output_mode": output_mode})));
    }
    let snapshot_id = args.get("snapshot_id").and_then(Value::as_str);
    let cache_key = sha256_bytes(format!("{}|{}|{}|{}|{}", grep, scope.to_string_lossy(), pattern, output_mode, args.get("ignore").cloned().unwrap_or(Value::Null)).as_bytes());
    let mut cache_hit = false;
    let mut cached_snapshot: Option<String> = None;
    let all_matches = if let Some(snapshot) = snapshot_id {
        state.snapshots.get(snapshot).cloned().ok_or_else(|| FsError::new(format!("{operation}_snapshot_not_found"), format!("{operation}_snapshot_not_found: {snapshot}"), json!({"snapshot_id": snapshot})))?
    ...
    let snapshot = if let Some(snapshot) = snapshot_id {
        ...
        state.cache.insert(cache_key, (id.clone(), all_matches.clone()));
        state.snapshots.insert(id.clone(), all_matches.clone());
        Some(id)
    } else { snapshot_id.map(str::to_string) };
    if cache_policy == "auto" && snapshot_id.is_none() && !cache_hit { cache_hit = true; }
    let page: Vec<String> = all_matches.iter().skip(offset).take(limit).cloned().collect();
    let has_more = offset + page.len() < all_matches.len();
    let mut value = Map::new();
    value.insert("schema".into(), json!(if grep { "local.filesystem.grep.v1" } else { "local.filesystem.glob.v1" }));
    value.insert("status".into(), json!("ok"));
    if grep { value.insert("output_mode".into(), json!(output_mode)); }
    ...
    value.insert("snapshot_id".into(), snapshot.clone().map(Value::String).unwrap_or(Value::Null));
    value.insert("requested_snapshot_id".into(), snapshot_id.map(|value| json!(value)).unwrap_or(Value::Null));
    value.insert("snapshot_complete".into(), json!(true));
    value.insert("cache_memory_bytes".into(), json!(all_matches.iter().map(|value| value.len()).sum::<usize>()));
    ...
    value.insert("timeout_ms".into(), args.get("timeout_ms").cloned().unwrap_or(json!(SEARCH_TIMEOUT_MS)));
    value.insert("freshness".into(), freshness(&scope));
    value.insert("has_more".into(), json!(has_more));
    value.insert("next_offset".into(), if has_more { json!(offset + page.len()) } else { Value::Null });
    if grep {
        value.insert("matches_format".into(), json!("human"));
        value.insert("matches".into(), json!(page.iter().map(|line| render_grep(line, output_mode)).collect::<Vec<_>>()));
        value.insert("match_objects_authoritative".into(), json!(true));
        value.insert("match_objects".into(), Value::Array(page.iter().map(|line| grep_match_object(line, output_mode)).collect()));
    } else {
...
fn run_search_command(scope: &Path, pattern: &str, args: &Value, grep: bool, output_mode: &str, operation: &str) -> Result<Vec<String>, FsError> {
    let mut rg_args = Vec::new();
    if grep {
        rg_args.extend(["--field-match-separator", "\u{1f}", "--with-filename"].iter().map(|value| value.to_string()));
        rg_args.push(match output_mode { "content" => "-n", "count_matches" => "-c", _ => "-l" }.to_string());
    } else {
        rg_args.extend(["--files", "--hidden", "--no-ignore"].iter().map(|value| value.to_string()));
        rg_args.push("-g".to_string());
        rg_args.push(pattern.to_string());
    }
    let ignores = if grep { DEFAULT_GREP_IGNORES } else { DEFAULT_GLOB_IGNORES };
    ...
    if grep {
        rg_args.extend(["--", pattern, &scope.to_string_lossy()].iter().map(|value| value.to_string()));
    } else {
        rg_args.push(scope.to_string_lossy().to_string());
    }
    run_rg(&rg_args, args.get("timeout_ms").and_then(Value::as_u64).unwrap_or(SEARCH_TIMEOUT_MS), operation)
}

fn repository_inventory(state: &mut State, args: &Value) -> Result<Value, FsError> {
    let include_generated = args.get("include_generated").and_then(Value::as_bool).unwrap_or(false);
    let mut cloned = args.clone();
    let object = cloned.as_object_mut().ok_or_else(|| FsError::new("arguments_must_be_object", "arguments_must_be_object", json!({})))?;
    ...
    if !include_generated {
        ...
        if let Some(array) = ignores.as_array_mut() {
            array.extend(["**/.ai/runtime/**", "**/.ai/tmp/**", "**/.ai/output/**", "**/.narada/runtime/**", "**/.narada/tmp/**", "**/.narada/local-filesystem-mcp/patch-outcomes/**", "**/.tmp-tests/**"].iter().map(|value| json!(value)));
        }
    }
    ...
    if let Some(items) = matches.as_array() {
        for item in items.iter().filter_map(Value::as_str) {
            ...
            if classification == "generated_artifact" { generated.push(item); } else { candidates.push(item); }
        }
    }
    let mut result = value.as_object().cloned().unwrap_or_default();
    result.insert("schema".into(), json!("local.filesystem.repository_inventory.v1"));
    result.insert("directory".into(), json!(args.get("directory").and_then(Value::as_str).unwrap_or(".")));
    result.insert("pattern".into(), json!(args.get("pattern").and_then(Value::as_str).unwrap_or("**/*")));
    ...
    result.insert("generated_artifacts_excluded_by_default".into(), json!(!include_generated));
...
fn file_metrics(state: &mut State, args: &Value) -> Result<Value, FsError> {
    if args.get("directory").and_then(Value::as_str).is_some() && args.get("root").and_then(Value::as_str).is_some() {
        return Err(FsError::new("file_metrics_directory_ambiguous", "file_metrics_directory_ambiguous", json!({"remediation": "Pass either directory or root, not both."})));
    }
    let directory_arg = args.get("directory").and_then(Value::as_str).or_else(|| args.get("root").and_then(Value::as_str)).unwrap_or(".");
    let (directory, root) = resolve_allowed(state, Some(directory_arg), "fs_file_metrics")?;
    let mut glob_args = args.clone();
    let object = glob_args.as_object_mut().ok_or_else(|| FsError::new("arguments_must_be_object", "arguments_must_be_object", json!({})))?;
    ...
    loop {
        all_matches.extend(all.get("matches").and_then(Value::as_array).cloned().unwrap_or_default());
        if all.get("has_more").and_then(Value::as_bool) != Some(true) { break; }
        page_offset = all.get("next_offset").and_then(Value::as_i64).unwrap_or(page_offset + 500);
        if all_matches.len() > 10_000 { break; }
        if let Some(object) = glob_args.as_object_mut() {
            object.insert("offset".into(), json!(page_offset));
            object.insert("limit".into(), json!(500));
        }
    ...
    let metrics_snapshot_id = args.get("snapshot_id").and_then(Value::as_str)
        .map(str::to_string)
        .or_else(|| all.get("snapshot_id").and_then(Value::as_str).map(str::to_string));
    let offset = integer(args, "offset").unwrap_or(0).max(0) as usize;
    let limit = integer(args, "limit").unwrap_or(100).max(1).min(100) as usize;
    let max_file = integer(args, "max_bytes_per_file").unwrap_or(8 * 1024 * 1024).max(1) as u64;
    let max_total = integer(args, "max_total_scan_bytes").unwrap_or(256 * 1024 * 1024).max(1) as u64;
    let mut reserved = 0_u64;
    let mut files = Vec::new();
    for item in matches.iter().skip(offset).take(limit) {
        let Some(raw) = item.as_str() else { continue };
        let path = PathBuf::from(raw);
        let metadata = match fs::metadata(&path) { Ok(value) if value.is_file() => value, _ => continue };
    ...
    result.insert("next_offset".into(), if has_more { json!(offset + files.len()) } else { Value::Null });
    result.insert("order".into(), json!("ripgrep_traversal"));
    result.insert("cache_hit".into(), json!(false));
    result.insert("cache_policy".into(), json!(args.get("cache_policy").and_then(Value::as_str).unwrap_or("auto")));
    result.insert("snapshot_id".into(), metrics_snapshot_id.clone().map(Value::String).unwrap_or(Value::Null));
    result.insert("requested_snapshot_id".into(), args.get("snapshot_id").cloned().unwrap_or(Value::Null));
    result.insert("snapshot_complete".into(), json!(metrics_snapshot_id.is_some()));
    result.insert("timeout_ms".into(), args.get("timeout_ms").cloned().unwrap_or(json!(10_000)));
...
fn patch_outcome(state: &State, args: &Value) -> Result<Value, FsError> {
    let operation = args.get("operation_id").and_then(Value::as_str).ok_or_else(|| FsError::new("operation_id_required", "operation_id_required", json!({})))?;
    let path = state.output_root.join(".narada").join("local-filesystem-mcp").join("patch-outcomes").join(format!("{operation}.json"));
    let text = fs::read_to_string(&path).map_err(|_| FsError::new("fs_patch_outcome_not_found", format!("fs_patch_outcome_not_found: {operation}"), json!({"operation_id": operation, "path": path})))?;
...
fn list_tools(mode: &str) -> Vec<Value> {
    let mut names = vec![
        ...
        ("fs_stat", "Return file or directory metadata under an allowed root."),
        ("fs_glob_search", "List files under an allowed root using ripgrep file globbing."),
        ("fs_grep_search", "Search file contents under an allowed root using ripgrep."),
...
fn resolve_allowed(state: &State, input: Option<&str>, operation: &str) -> Result<(PathBuf, PathBuf), FsError> {
    let input = input.filter(|value| !value.trim().is_empty()).ok_or_else(|| FsError::new("path_required", "path_required", json!({"operation": operation})))?;
    let base = state.allowed_roots.first().cloned().ok_or_else(|| FsError::new("filesystem_mcp_requires_at_least_one_allowed_root", "filesystem_mcp_requires_at_least_one_allowed_root", json!({})))?;
    let candidate = if Path::new(input).is_absolute() { absolute(PathBuf::from(input)) } else { absolute(base.join(input)) };
    let root = state.allowed_roots.iter().find(|root| within(root, &candidate)).cloned();
...
fn relative_path(root: &Path, path: &Path) -> String {
    path.strip_prefix(root).map(|value| value.to_string_lossy().replace('\\', "/")).unwrap_or_else(|_| path.to_string_lossy().replace('\\', "/"))
}
...
fn mtime_iso(metadata: &fs::Metadata) -> String {
    let duration = metadata.modified().ok().and_then(|value| value.duration_since(std::time::UNIX_EPOCH).ok());
    if let Some(value) = duration {
        if let Ok(date) = OffsetDateTime::from_unix_timestamp(value.as_secs() as i64) {
            return date.format(&Rfc3339).unwrap_or_else(|_| "1970-01-01T00:00:00Z".to_string());
        }
    }
...
fn directory_fingerprint(path: &Path, root: &Path) -> (usize, usize, String, bool) {
    ...
    (direct_count, entries.len(), sha256_bytes(entries.join("\n").as_bytes()), truncated)
}

fn walk_directory(path: &Path, root: &Path, entries: &mut Vec<String>, truncated: &mut bool) {
    if entries.len() >= 5000 { *truncated = true; return; }
    ...
    for entry in children {
        ...
        let Ok(metadata) = fs::metadata(&child) else { continue };
        let kind = if metadata.is_dir() {"directory"} else if metadata.is_file() {"file"} else {"other"};
        entries.push(format!("{}\t{}\t{}\t{}", relative_path(root, &child), kind, metadata.len(), metadata.modified().ok().and_then(|value| value.duration_since(std::time::UNIX_EPOCH).ok()).map(|value| value.as_millis()).unwrap_or(0)));
        if metadata.is_dir() { walk_directory(&child, root, entries, truncated); }
    }
}

fn run_rg(args: &[String], timeout: u64, operation: &str) -> Result<Vec<String>, FsError> {
    let started = std::time::Instant::now();
    let output = Command::new("rg").args(args).output().map_err(|error| FsError::new(format!("{operation}_failed"), format!("{operation}_failed: {error}"), json!({"operation": operation})))?;
    if started.elapsed().as_millis() as u64 > timeout { return Err(FsError::new(format!("{operation}_timed_out"), format!("{operation}_timed_out"), json!({"operation": operation, "timeout_ms": timeout}))); }
    if output.status.code().unwrap_or(2) > 1 { return Err(FsError::new(format!("{operation}_failed"), format!("{operation}_failed: {}", String::from_utf8_lossy(&output.stderr).trim()), json!({"operation": operation, "status": output.status.code()}))); }
    Ok(String::from_utf8_lossy(&output.stdout).lines().filter(|line| !line.trim().is_empty()).map(|line| line.trim_end_matches('\r').to_string()).collect())
}

fn render_grep(line: &str, mode: &str) -> String {
    let fields: Vec<&str> = line.split('\u{1f}').collect();
    match mode {
        "count_matches" => if fields.len() >= 2 { format!("{}: {}", fields[0], fields[1]) } else { line.to_string() },
        "content" => if fields.len() >= 3 { format!("{}:{}:{}", fields[0], fields[1], fields[2..].join("\u{1f}")) } else { line.to_string() },
...
fn grep_match_object(line: &str, mode: &str) -> Value {
    let fields: Vec<&str> = line.split('\u{1f}').collect();
    match mode {
        "count_matches" => json!({"path": fields.first().copied().unwrap_or(line), "count": fields.get(1).and_then(|value| value.parse::<u64>().ok()), "raw": line}),
        "content" => json!({"path": fields.first().copied().unwrap_or(line), "line": fields.get(1).and_then(|value| value.parse::<u64>().ok()), "text": fields.get(2).copied().unwrap_or(""), "raw": line}),
        _ => json!({"path": line, "raw": line}),
    }
}

fn classify(path: &str) -> &'static str {
    let normalized = format!("/{}/", path.replace('\\', "/").trim_matches('/')).to_ascii_lowercase();
    if GENERATED_MARKERS.iter().any(|marker| normalized.contains(marker)) { "generated_artifact" } else { "candidate_source" }
}

fn count_lines(path: &Path) -> io::Result<(usize, bool)> {
    let bytes = fs::read(path)?;
    if bytes.contains(&0) { return Ok((0, true)); }
    let text = String::from_utf8_lossy(&bytes).replace("\r\n", "\n");
    let mut lines: Vec<&str> = text.split('\n').collect();
    if lines.last() == Some(&"") { lines.pop(); }
    Ok((lines.len(), false))
...
fn aggregate_metrics(files: &[Value]) -> Value {
    ...
    for file in files {
        ...
        if let Some(value) = file.get("line_count").and_then(Value::as_u64) { lines += value; } else {
            exact = false;
            match file.get("line_count_status").and_then(Value::as_str).unwrap_or_default() {
                "binary" => binary += 1,
...
pub(crate) fn read_message<R: BufRead>(reader: &mut R) -> io::Result<Option<(Value, bool)>> {
    ...
    if first.to_ascii_lowercase().starts_with("content-length:") {
        let length = first.split(':').nth(1).and_then(|value| value.trim().parse::<usize>().ok()).unwrap_or(0);
        ...
        return serde_json::from_slice(&body).map(|value| Some((value, true))).map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error));
    }
