...
pub fn run(args: &[String]) -> Result<(), String> {
        ...
        loop {
            match read_message(&mut reader) {
                Ok(Some((request, framed))) => {
                    if reader_tx.send(Event::Request(request, framed)).is_err() { return; }
                }
                Ok(None) | Err(_) => {
    ...
    while let Ok(event) = events_rx.recv() {
        match event {
            Event::Request(request, framed) => {
                ...
                if method == "notifications/cancelled" {
                    let request_id = request.get("params").and_then(|params| params.get("requestId")).map(value_key).unwrap_or_default();
                    if let Some(token) = active.get(&request_id) { token.store(true, Ordering::Release); }
                    continue;
                }
        ...
        if input_closed && active.is_empty() { break; }
    }
    Ok(())
}
...
fn parse_state(args: &[String]) -> Result<State, String> {
    ...
    let site_root = absolute(PathBuf::from(site_root.unwrap_or_else(|| allowed_roots[0].to_string_lossy().to_string())));
    let storage_root = absolute(PathBuf::from(storage_root.unwrap_or_else(|| allowed_roots[0].to_string_lossy().to_string())));
    Ok(State {
        allowed_roots,
...
fn handle_request(state: &State, request: &Value, cancellation: Option<Arc<AtomicBool>>) -> Option<Value> {
    let method = request.get("method").and_then(Value::as_str).unwrap_or_default();
...
fn call_tool(state: &State, params: &Value, cancellation: Option<Arc<AtomicBool>>) -> Result<Value, StructuredError> {
    ...
    let payload = match name {
        ...
        "structured_command_powershell_parse_check" => powershell_parse_check(state, args, cancellation),
        "structured_command_start" => Err(StructuredError::new("structured_command_background_not_available", "structured_command_background_not_available", json!({"remediation": "Use the JavaScript structured-command surface for durable background execution."}))),
...
fn powershell_parse_check(state: &State, args: &Value, cancellation: Option<Arc<AtomicBool>>) -> Result<Value, StructuredError> {
    let object = args.as_object().cloned().unwrap_or_default();
    let path_value = object.get("path").and_then(Value::as_str).unwrap_or_default();
    let path = resolve_path(path_value, &state.allowed_roots[0]);
    if !path.to_string_lossy().to_ascii_lowercase().ends_with(".ps1") {
        return Err(StructuredError::new("structured_command_powershell_parse_check_requires_ps1", "structured_command_powershell_parse_check_requires_ps1", json!({"path": path_value})));
    }
    if !inside_any_root(&path, &state.allowed_roots) {
        return Err(StructuredError::new("structured_command_powershell_parse_check_path_outside_allowed_roots", "structured_command_powershell_parse_check_path_outside_allowed_roots", json!({"path": path.to_string_lossy()})));
    }
    if !path.is_file() {
        return Err(StructuredError::new("structured_command_powershell_parse_check_file_not_found", "structured_command_powershell_parse_check_file_not_found", json!({"path": path.to_string_lossy()})));
    }
    let cwd = object.get("working_directory").and_then(Value::as_str).map(|value| resolve_path(value, &state.allowed_roots[0])).unwrap_or_else(|| path.parent().unwrap_or(&state.allowed_roots[0]).to_path_buf());
    if !inside_any_root(&cwd, &state.allowed_roots) {
        return Err(StructuredError::new("structured_command_powershell_parse_check_cwd_outside_allowed_roots", "structured_command_powershell_parse_check_cwd_outside_allowed_roots", json!({"working_directory": cwd.to_string_lossy()})));
    }
    let timeout = object.get("timeout_ms").and_then(Value::as_u64).unwrap_or(30_000).clamp(1, state.max_timeout_ms);
...
fn elevated_refusal(args: &Value) -> Result<Value, StructuredError> {
    ...
    let cwd = object.get("working_directory").and_then(Value::as_str).unwrap_or_default();
    ...
    if dry_run {
        return Ok(json!({"schema": "narada.structured_command.elevated_window_result.v0", "status": "planned", "executed": false, "command": command, "args": command_args, "working_directory": cwd, "wait": wait, "broker": broker, "note": "Native Rust exposes the broker plan but does not launch a privileged process."}));
    }
    Ok(json!({"schema": "narada.structured_command.elevated_window_result.v0", "status": "refused", "executed": false, "command": command, "args": command_args, "working_directory": cwd, "wait": wait, "refusal_reasons": ["native_elevation_not_enabled"], "remediation_hints": ["Use the JavaScript structured-command surface for confirmed Windows UAC execution."], "broker": broker}))
}
...
fn ps_array_literal(args: &[String]) -> String {
    if args.is_empty() { "@()".to_string() } else { format!("@({})", args.iter().map(|value| ps_single_quote(value)).collect::<Vec<_>>().join(", ")) }
}
...
fn parse_ref(value: &str, kind: &str) -> Result<String, StructuredError> {
    ...
    let Some(id) = value.strip_prefix(&prefix) else {
        return Err(StructuredError::new(format!("structured_command_invalid_{kind}_ref"), format!("structured_command_invalid_{kind}_ref"), json!({"ref": value, "expected_kind": kind})));
    };
    if id.len() < 8 || !id.chars().all(|character| character.is_ascii_alphanumeric() || character == '_' || character == '-') {
        return Err(StructuredError::new(format!("structured_command_invalid_{kind}_ref"), format!("structured_command_invalid_{kind}_ref"), json!({"ref": value, "expected_kind": kind})));
    }
    Ok(id.to_string())
}
...
fn value_strings(value: Option<&Value>) -> Vec<String> {
    value.and_then(Value::as_array).map(|values| values.iter().filter_map(Value::as_str).map(str::to_string).collect()).unwrap_or_default()
}

fn execution_path(state: &State, id: &str) -> PathBuf {
    state.storage_root.join("executions").join(format!("{id}.json"))
}
...
fn write_json_record(path: &Path, value: &Value) -> Result<(), StructuredError> {
    if let Some(parent) = path.parent() { fs::create_dir_all(parent).map_err(|error| StructuredError::new("structured_command_persistence_failed", error.to_string(), json!({"path": parent.to_string_lossy()})))?; }
    let serialized = format!("{}\n", serde_json::to_string_pretty(value).map_err(|error| StructuredError::new("structured_command_persistence_failed", error.to_string(), json!({"path": path.to_string_lossy()})))?);
    fs::write(path, serialized).map_err(|error| StructuredError::new("structured_command_persistence_failed", error.to_string(), json!({"path": path.to_string_lossy()})))
}

fn read_json_record(path: &Path) -> Result<Value, StructuredError> {
    let bytes = fs::read(path).map_err(|_| StructuredError::new("structured_command_ref_not_found", "structured_command_ref_not_found", json!({"path": path.to_string_lossy()})))?;
    serde_json::from_slice(&bytes).map_err(|error| StructuredError::new("structured_command_ref_invalid_json", error.to_string(), json!({"path": path.to_string_lossy()})))
}
...
fn update_execution_record(state: &State, reference: &str, payload: &Value) -> Result<(), StructuredError> {
    let id = parse_ref(reference, "execution")?;
    let path = execution_path(state, &id);
    let existing = read_json_record(&path)?;
    let created_at = existing.get("created_at").cloned().unwrap_or_else(|| json!(now_rfc3339()));
...
fn read_execution_record(state: &State, reference: &str) -> Result<Value, StructuredError> {
    ...
    record.get("result").cloned().ok_or_else(|| StructuredError::new("structured_command_ref_invalid_json", "structured_command_execution_result_missing", json!({"ref": reference})))
}

fn create_input_record(state: &State, args: &Value) -> Result<Value, StructuredError> {
    let object = args.as_object().cloned().unwrap_or_default();
    let id = object.get("input_id").and_then(Value::as_str).map(str::to_string).unwrap_or_else(|| unique_id("i"));
    if id.len() < 8 || !id.chars().all(|character| character.is_ascii_alphanumeric() || character == '_' || character == '-') {
        return Err(StructuredError::new("structured_command_invalid_ref_id", "structured_command_invalid_ref_id", json!({"input_id": id})));
    }
    let input = json!({
    ...
    Ok(json!({"schema": "narada.structured_command.input_create_result.v0", "status": "created", "input_ref": reference, "sha256": sha256_json(record.get("input").unwrap_or(&Value::Null))}))
}
...
fn read_input_record(state: &State, reference: &str) -> Result<Value, StructuredError> {
    ...
    record.get("input").cloned().ok_or_else(|| StructuredError::new("structured_command_ref_invalid_json", "structured_command_input_missing", json!({"ref": reference})))
}

fn unique_id(prefix: &str) -> String {
    static COUNTER: std::sync::atomic::AtomicU64 = std::sync::atomic::AtomicU64::new(1);
    let nanos = SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_nanos();
    format!("{prefix}_{}_{}_{}", std::process::id(), nanos, COUNTER.fetch_add(1, Ordering::Relaxed))
}
...
fn execute(state: &State, args: &Value, cancellation: Option<Arc<AtomicBool>>) -> Result<Value, StructuredError> {
    ...
    if let Some(reference) = args_object.get("execution_ref").and_then(Value::as_str) {
        ...
        return Ok(page_execution(&payload, &args_object, Some(reference.to_string())));
    }
    let effective_args = if let Some(reference) = args_object.get("input_ref").and_then(Value::as_str) {
        read_input_record(state, reference)?.as_object().cloned().unwrap_or_default()
    } else {
        args_object.clone()
    };
    let command = normalize_command(effective_args.get("command").and_then(Value::as_str).unwrap_or_default());
    let command_args = effective_args.get("args").and_then(Value::as_array).map(|values| values.iter().map(|value| value.as_str().unwrap_or_default().to_string()).collect::<Vec<_>>()).unwrap_or_default();
    let working_directory = effective_args.get("working_directory").and_then(Value::as_str).map(|value| resolve_path(value, &state.allowed_roots[0])).unwrap_or_else(|| state.allowed_roots[0].clone());
    let timeout_ms = effective_args.get("timeout_ms").and_then(Value::as_u64).unwrap_or(60_000).clamp(1, state.max_timeout_ms);
    let test_scope = effective_args.get("test_scope").and_then(Value::as_str).unwrap_or_else(|| infer_test_scope(&command, &command_args));
    let expected_cost = effective_args.get("expected_cost").and_then(Value::as_str).unwrap_or_else(|| infer_expected_cost(test_scope));
    ...
    let result = run_process(&command, &command_args, &working_directory, timeout_ms, state.max_output_bytes, cancellation, &state.env);
    let payload = execution_payload(&command, &command_args, &working_directory, &started_at, timeout_ms, posture, result, "synchronous", true, args_object.get("input_ref").cloned().unwrap_or(Value::Null));
    audit(state, &payload);
...
fn infer_test_scope(command: &str, args: &[String]) -> &'static str {
    if command.eq_ignore_ascii_case("pnpm") && args.iter().any(|value| value.eq_ignore_ascii_case("test")) {
        if args.iter().any(|value| value == "--filter") { "focused" } else { "broad" }
    } else if command.eq_ignore_ascii_case("npm") && args.iter().any(|value| value.eq_ignore_ascii_case("test")) {
        "broad"
    } else { "unknown" }
}
...
fn execution_payload(command: &str, args: &[String], cwd: &Path, started_at: &str, timeout_ms: u64, posture: Value, result: ProcessResult, mode: &str, wait_for_completion: bool, input_ref: Value) -> Value {
    json!({
...
fn page_execution(payload: &Value, args: &Map<String, Value>, reference: Option<String>) -> Value {
    ...
    if payload.get("executed").and_then(Value::as_bool) == Some(false) {
        ...
        if let Some(object) = result.as_object_mut() { object.insert("execution_ref".to_string(), reference.clone().map(Value::String).unwrap_or(Value::Null)); }
        return result;
    }
    let stdout = payload.get("stdout").and_then(Value::as_str).unwrap_or_default();
    let stderr = payload.get("stderr").and_then(Value::as_str).unwrap_or_default();
    let stdout_offset = args.get("stdout_offset").and_then(Value::as_u64).unwrap_or(0) as usize;
    let stderr_offset = args.get("stderr_offset").and_then(Value::as_u64).unwrap_or(0) as usize;
    let stdout_limit = args.get("stdout_limit").and_then(Value::as_u64).unwrap_or(1000).clamp(1, 20_000) as usize;
    let stderr_limit = args.get("stderr_limit").and_then(Value::as_u64).unwrap_or(1000).clamp(1, 20_000) as usize;
    ...
    if let Some(object) = result.as_object_mut() {
        object.insert("execution_ref".to_string(), reference.clone().map(Value::String).unwrap_or(Value::Null));
        ...
        object.insert("stdout_next_offset".to_string(), stdout_page.1.map(|value| json!(value)).unwrap_or(Value::Null));
        object.insert("stderr_next_offset".to_string(), stderr_page.1.map(|value| json!(value)).unwrap_or(Value::Null));
        object.insert("stdout_output_truncated".to_string(), json!(stdout_page.1.is_some()));
        object.insert("stderr_output_truncated".to_string(), json!(stderr_page.1.is_some()));
        object.insert("stdout_char_length".to_string(), json!(stdout.chars().count()));
        object.insert("stderr_char_length".to_string(), json!(stderr.chars().count()));
        object.insert("page_source".to_string(), json!(if persisted { "persisted_execution" } else { "new_execution" }));
    }
    result
}
...
fn tool_result(state: &State, payload: Value, tool_name: &str) -> Result<Value, StructuredError> {
    ...
    if text.chars().count() <= 4_000 {
        return Ok(json!({"content": [{"type": "text", "text": text, "annotations": {"audience": ["assistant"]}}], "structuredContent": payload}));
    }
...
fn materialize_output(state: &State, tool_name: &str, payload: &Value, text: &str) -> Result<(String, usize), StructuredError> {
    let id = unique_id("o");
    let reference = format!("mcp_output:{id}");
    let path = state.site_root.join(".ai").join("tmp").join("mcp-outputs").join("workspace").join(format!("{id}.json"));
    ...
    if serialized.len() > 20 * 1024 * 1024 { return Err(StructuredError::new("mcp_output_too_large", "mcp_output_too_large", json!({"ref": reference}))); }
    write_json_record(&path, &record)?;
...
fn output_show(state: &State, args: &Value) -> Result<Value, StructuredError> {
    let object = args.as_object().cloned().unwrap_or_default();
    let reference = object.get("ref").and_then(Value::as_str).or_else(|| object.get("output_ref").and_then(Value::as_str)).unwrap_or_default();
    let Some(id) = reference.strip_prefix("mcp_output:") else { return Err(StructuredError::new("output_ref_invalid", "output_ref_invalid", json!({"ref": reference}))); };
    if id.len() < 8 || !id.chars().all(|character| character.is_ascii_alphanumeric() || character == '_' || character == '-') { return Err(StructuredError::new("output_ref_invalid", "output_ref_invalid", json!({"ref": reference}))); }
    let record = read_json_record(&state.site_root.join(".ai").join("tmp").join("mcp-outputs").join("workspace").join(format!("{id}.json")))?;
    if record.get("schema").and_then(Value::as_str) != Some("narada.mcp_output_ref.v1") { return Err(StructuredError::new("output_ref_schema_unsupported", "output_ref_schema_unsupported", json!({"ref": reference}))); }
    ...
    let limit = object.get("limit").and_then(Value::as_u64).unwrap_or(20_000).clamp(1, 20_000) as usize;
...
fn run_process(command: &str, args: &[String], cwd: &Path, timeout_ms: u64, max_output_bytes: usize, cancellation: Option<Arc<AtomicBool>>, environment: &std::collections::HashMap<String, String>) -> ProcessResult {
    let child_result = Command::new(command).args(args).current_dir(cwd).envs(environment).stdin(Stdio::null()).stdout(Stdio::piped()).stderr(Stdio::piped()).spawn();
    ...
    let status = loop {
        match child.try_wait() {
            ...
            Ok(None) if cancellation.as_ref().is_some_and(|token| token.load(Ordering::Acquire)) => { cancelled = true; kill_child(&mut child); break child.wait().ok(); }
    ...
    ProcessResult { exit_code: status.and_then(|status| status.code()), timed_out, cancelled, stdout: String::from_utf8_lossy(&stdout_result.0).to_string(), stderr: String::from_utf8_lossy(&stderr_result.0).to_string(), stdout_truncated: stdout_result.1, stderr_truncated: stderr_result.1 }
}
