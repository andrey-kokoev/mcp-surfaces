...
const executable = resolve(process.env.NARADA_NATIVE_STRUCTURED_COMMAND_TEST_EXECUTABLE ?? resolveNativeArtifact(packageRoot, 'narada-mcp-runtime.exe') ?? resolve(packageRoot, 'dist', 'native', 'narada-mcp-runtime.exe'));
