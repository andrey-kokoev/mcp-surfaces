...
const SURFACE_ID = "site-inbox";
const GUIDANCE_TOOL = "inbox_guidance";
const PURPOSE = "Governed site inbox intake and triage.";
...
export function buildGuidanceResult(args: GuidanceRecord = {}): GuidanceRecord {
  ...
  return {
    ...
    examples: [
      { intent: 'First use', call: 'inbox_guidance({})' },
      { intent: 'Tool-specific help', call: "inbox_guidance({ tool: \"<tool_name>\" })" },
      { intent: 'Workflow-specific help', call: "inbox_guidance({ workflow: \"<workflow_name>\" })" }
    ],
    anti_patterns: [
