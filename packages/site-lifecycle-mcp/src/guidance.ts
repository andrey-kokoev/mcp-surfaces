...
const SURFACE_ID = "site-lifecycle";
const GUIDANCE_TOOL = "site_lifecycle_guidance";
const PURPOSE = "Governed site creation planning, lifecycle inspection, relations, and configuration mutation.";
...
export function buildGuidanceResult(args: GuidanceRecord = {}): GuidanceRecord {
  ...
  return {
    ...
    examples: [
      { intent: 'First use', call: 'site_lifecycle_guidance({})' },
      { intent: 'Tool-specific help', call: "site_lifecycle_guidance({ tool: \"<tool_name>\" })" },
      { intent: 'Workflow-specific help', call: "site_lifecycle_guidance({ workflow: \"<workflow_name>\" })" }
    ],
    anti_patterns: [
