...
const SURFACE_ID = "site-registry";
const GUIDANCE_TOOL = "site_registry_guidance";
const PURPOSE = "Read-only inspection and reconciliation planning for the canonical cross-site Narada Site Registry.";
...
export function buildGuidanceResult(args: GuidanceRecord = {}): GuidanceRecord {
  ...
  return {
    ...
    examples: [
      { intent: 'First use', call: 'site_registry_guidance({})' },
      { intent: 'Tool-specific help', call: "site_registry_guidance({ tool: \"<tool_name>\" })" },
      { intent: 'Workflow-specific help', call: "site_registry_guidance({ workflow: \"<workflow_name>\" })" }
    ],
    anti_patterns: [
