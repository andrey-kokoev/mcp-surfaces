...
import { openTaskLifecycleStoreWithDiscipline, taskLifecycleDbHealth } from '../task-lifecycle/sqlite-discipline.mjs';
import { pollInboxBridge } from '@narada2/task-lifecycle-mcp/task-lifecycle-runtime/inbox-bridge';
import { dispatchPendingDirectives, getResidentStatus } from '../task-lifecycle/dispatch-directives.mjs';
import { taskLifecycleTools } from '../task-lifecycle/task-mcp-tool-registry.mjs';
import { loadSonarEmailResidentOperatingPolicy } from './operating-loop-policy.mjs';
import {
...
} from './sonar-email-resident-constants.mjs';
import {
...
export { SONAR_EMAIL_RESIDENT_LOOP_ID } from './sonar-email-resident-constants.mjs';
const MAILBOX_PROOF_FRESHNESS_MS = 24 * 60 * 60_000;

export async function runSonarEmailResidentLoop(cwd, options = {}) {
  ...
  try {
    ...
    const ticketTaskReconciliation = dryRun || drain || !sourceSync
      ? {
    ...
    const residentSupervisor = dryRun || !options.ensureResident
...
async function runSourceSync(siteRoot, { dryRun, runner } = {}) {
  ...
  if ((result.status ?? 1) !== 0) {
    const error = new Error(`source sync failed: ${result.stderr || result.stdout || result.status}`);
...
async function runTicketTaskReconcile(siteRoot, { dryRun, limit, preferredRole, runner } = {}) {
  if (typeof runner === 'function') {
  ...
  if (result.error) throw result.error;
  if ((result.status ?? 1) !== 0) {
    const error = new Error(`ticket task reconcile failed: ${result.stderr || result.stdout || result.status}`);
    error.stdout = result.stdout;
    error.stderr = result.stderr;
    throw error;
  }
...
export function listSonarEmailResidentLoopRuns(cwd, options = {}) {
...
export function sonarEmailResidentOperatingLayerStatus(cwd, options = {}) {
  const siteRoot = resolve(cwd);
  ...
  const agentCliControlPath = resident.carrier?.preferred_interactive?.controlPath ?? (resident.carrier?.runtime === 'agent-cli' ? resident.carrier?.controlPath ?? null : null);
  const agentCliReady = Boolean(agentCliControlPath && existsSync(agentCliControlPath))
    && (resident.carrier?.preferred_interactive?.status === 'available' || resident.carrier?.runtime === 'agent-cli');
  const primaryRuntimeReady = resident.carrier?.runtime === 'agent-cli'
    && resident.carrier?.preference === 'interactive_agent_cli'
    && proofDriver !== true;
  ...
  return {
    ...
    resident: {
      ...
      runtime: resident.carrier?.runtime ?? null,
      preference: resident.carrier?.preference ?? null,
      ...
      preferred_runtime_selected: resident.carrier?.preference === 'interactive_agent_cli',
      carrier_session_id: resident.carrier?.carrierSessionId ?? null,
...
export function sonarEmailResidentReadiness(cwd, options = {}) {
  const siteRoot = resolve(cwd);
  ...
  const projection = options.projectionDriftResult ?? runReadinessJsonCommand(siteRoot, [
    process.execPath,
    [join(siteRoot, 'tools', 'task-lifecycle', 'detect-projection-drift.mjs'), siteRoot],
  ]);
...
export function sonarEmailResidentCoherence(cwd, options = {}) {
...
function readinessGate(gate, ok, detail = {}) {
  return {
    gate,
    ...detail,
...
function recentSurfacePolicyNoise(siteRoot, options = {}) {
...
function latestResidentMailboxProof(siteRoot, options = {}) {
  const dir = residentMailboxProofDir(siteRoot);
...
export function sonarResidentStatus(cwd, options = {}) {
  const siteRoot = resolve(cwd);
...
export function sonarResidentPending(cwd, options = {}) {
  const siteRoot = resolve(cwd);
  const lifecycleStore = openTaskLifecycleStoreWithDiscipline(siteRoot, { write: false });
  try {
...
function sonarResidentPendingFromDb(db, options = {}) {
...
export function sonarResidentReceipts(cwd, options = {}) {
  const siteRoot = resolve(cwd);
  const lifecycleStore = openTaskLifecycleStoreWithDiscipline(siteRoot, { write: false });
  try {
    const limit = Number(options.limit ?? 25);
...
export function sonarResidentOutcomes(cwd, options = {}) {
  const store = openSiteLoopStore(resolve(cwd));
  try {
...
function residentBacklogSummaryFromDb(db, options = {}) {
...
async function reconcileReportedResidentTaskLifecycleState(siteRoot, options = {}) {
...
export async function cleanupResidentFixtureResidue(cwd, options = {}) {
  const siteRoot = resolve(cwd);
  const dryRun = options.dryRun === true || options.dry_run === true;
  const limit = Number(options.limit ?? 100);
  const nowIso = options.nowIso ?? new Date().toISOString();
...
function stalePendingDirectives(directives, options = {}) {
...
function persistOperatingLayerAlerts(siteRoot, store, { runId, nowIso = new Date().toISOString(), requireFreshProductionProof = false } = {}) {
  const resident = sonarResidentStatus(siteRoot);
...
export function refuseResidentDirective(cwd, options = {}) {
...
export function sonarResidentCapabilities(cwd, options = {}) {
  const siteRoot = resolve(cwd);
...
export function ensureResidentCarrier(cwd, options = {}) {
  const siteRoot = resolve(cwd);
  ...
  if (['available', 'busy'].includes(before.status) && residentCarrierAcceptableForSupervisor(before)) {
    const redundantFallbackCleanup = before.carrier?.runtime === 'agent-cli'
...
function evaluateResidentRestartPolicy(siteRoot, options = {}) {
...
export async function runSonarResidentE2E(cwd, options = {}) {
  ...
  if (requireProductionProof) {
    ...
    const observedPreference = currentResident?.carrier?.preference ?? null;
    const observedPrimary = currentResident?.carrier?.runtime === 'agent-cli'
    ...
    if ((expectedCarrierPreference && observedPreference !== expectedCarrierPreference) || !observedPrimary) {
      return {
        ...
        fallback_carrier: currentResident?.carrier?.preference === 'agent_runtime_server_fallback'
          ? {
              status: 'available_but_insufficient_for_production_proof',
              runtime: currentResident?.carrier?.runtime ?? null,
              carrier_session_id: currentResident?.carrier?.carrierSessionId ?? null,
            }
          : null,
  ...
  let firstRun = await runSonarEmailResidentLoop(siteRoot, {
  ...
  const carrierPreferenceObserved = finalResident?.carrier?.preference ?? null;
  ...
  const mailboxProofRecord = mailboxProof && status === 'passed'
    ? recordResidentMailboxProof(siteRoot, {
        ...
        carrier_session_id: finalResident?.carrier?.carrierSessionId ?? null,
...
function simulateResidentFixtureCompletion(siteRoot, directiveIds, { at = new Date().toISOString() } = {}) {
...
export function listSonarLoopAttention(cwd, options = {}) {
  const store = openSiteLoopStore(resolve(cwd));
  try {
    return {
...
export function ackSonarLoopAttention(cwd, attentionId, options = {}) {
  const store = openSiteLoopStore(resolve(cwd));
  try {
    return {
...
export function recoverStaleResidentCarrier(cwd, options = {}) {
  const siteRoot = resolve(cwd);
...
export function cleanupResidentRuntime(cwd, options = {}) {
  const siteRoot = resolve(cwd);
...
export function sonarResidentProofPacket(cwd, options = {}) {
  const siteRoot = resolve(cwd);
  const resident = sonarResidentStatus(siteRoot);
  ...
  return {
    ...
    status: resident.status === 'available' || resident.status === 'busy'
      ...
      : 'attention_needed',
    mode: proofDriver ? 'proof_driver' : resident.carrier?.runtime === 'agent-cli' ? 'agent_cli_reasoning' : resident.carrier?.runtime === 'agent-runtime-server' ? 'agent_runtime_server_fallback_reasoning' : resident.carrier?.legacy_runtime === 'nars' ? 'agent_runtime_server_fallback_reasoning' : 'no_live_carrier',
    primary_runtime_ready: primaryRuntimeReady,
    agent_cli_ready: resident.carrier?.preferred_interactive?.status === 'available' || primaryRuntimeReady,
    agent_cli_control_path: resident.carrier?.preferred_interactive?.controlPath ?? (primaryRuntimeReady ? resident.carrier?.controlPath ?? null : null),
    preferred_runtime_selected: resident.carrier?.preference === 'interactive_agent_cli',
...
export async function runSonarResidentRecoveryDrill(cwd, options = {}) {
  const siteRoot = resolve(cwd);
  const fixtureId = options.id ?? `resident-recovery-drill-${Date.now()}`;
  const before = sonarResidentStatus(siteRoot);
  const retired = before.carrier?.carrierSessionId
    ? retireResidentCarrier(siteRoot, before.carrier.carrierSessionId, {
...
export function resolveSonarResidentOutcome(cwd, options = {}) {
  const directiveId = options.directiveId ?? options.directive_id;
  const store = openSiteLoopStore(resolve(cwd));
  try {
    return {
...
export function seedResidentWorkFixture(cwd, options = {}) {
...
export function cleanupResidentWorkFixture(cwd, options = {}) {
  const siteRoot = resolve(cwd);
  const id = options.id ?? 'resident_work_fixture';
  const envelopeId = `env_${sanitizeId(id)}`;
...
export async function superviseSonarEmailResidentLoop(cwd, options = {}) {
...
export async function runSonarEmailResidentSoak(cwd, options = {}) {
...
function recordSyntheticStep({ store, runId, stepId, status = 'ok', inputRefs = [], outputRefs = [], evidence = null }) {
...
function emitResidentBacklogRecoveryDirectives(siteRoot, options = {}) {
  const nowIso = options.nowIso ?? new Date().toISOString();
  const actionStaleMinutes = Number(options.actionStaleMinutes ?? 30);
...
function summarizeRun({ bridge, dispatch, steps }) {
...
export function runAgentOutcomeReconciliation(cwd, options = {}) {
  const nowIso = options.nowIso ?? new Date().toISOString();
...
function recordOutcomeForClassification(store, classification, { nowIso = new Date().toISOString() } = {}) {
...
function reconcileLoopEscalations(siteRoot, store, outcome, { runId, nowIso = new Date().toISOString() } = {}) {
  const created = [];
  const cleared = [];
  const observed = [];
...
function probeTaskLifecycleMcpTools(siteRoot) {
  ...
  const entrypoint = Array.isArray(server?.args)
    ? server.args.find((arg) => String(arg).replace(/\\/g, '/').includes('tools/task-lifecycle/task-mcp-server.mjs'))
    : null;
...
function defaultResidentLaunchRunner(siteRoot) {
  const launcher = join(siteRoot, 'tools', 'agent-start', 'start-agent.mjs');
  const host = join(siteRoot, 'tools', 'site-loop', 'agent-runtime-server-control-host.mjs');
...
function retireResidentCarrier(siteRoot, carrierSessionId, { reason, at } = {}) {
...
function isResidentCarrierLive(siteRoot, carrierSessionId, { staleAfterMs = 30000 } = {}) {
...
function isResidentDirectiveTarget(directive, options = {}) {
...
function mailboxProofDirectiveIds(siteRoot, run, beforeDirectiveIds = [], options = {}) {
...
function controlledMailboxSourceStatus(siteRoot, controlledSource, options = {}) {
