...
const siteRoot = mkdtempSync(join(tmpdir(), 'sonar-site-ops-mcp-protocol-'));
const serverPath = fileURLToPath(new URL('../src/site-ops-mcp-server.mjs', import.meta.url));
...
try {
  ...
  assert.equal(init.result.serverInfo.name, 'narada-sonar-site-ops-mcp');
...
console.log('sonar-site-ops-mcp protocol smoke ok');
