...
export function buildGuidanceResult(args: GuidanceRecord = {}): GuidanceRecord {
  ...
  return {
    ...
    examples: [
      { intent: 'First use', call: 'speech_guidance({})' },
      { intent: 'Tool-specific help', call: "speech_guidance({ tool: \"<tool_name>\" })" },
      { intent: 'Workflow-specific help', call: "speech_guidance({ workflow: \"<workflow_name>\" })" }
    ],
    anti_patterns: [
