...
export type WorkerRuntimeId = 'codex' | 'narada-agent-runtime-server';
...
export const WORKER_REQUIRED_MCP_SCOPE = 'local-site' as const;
const ENV_KEYS = [
  'PATH',
  'USERPROFILE',
  'HOME',
...
export function createWorkerPolicy(options: Record<string, unknown> = {}): WorkerPolicy {
  ...
  assertWorkerRuntime(defaultRuntime);
...
function runtimeList(value: unknown): WorkerRuntimeId[] {
  const list = stringList(value).map((runtime) => {
    assertWorkerRuntime(runtime);
    return runtime;
  });
  return list.length > 0 ? [...new Set(list)] : ['codex', 'narada-agent-runtime-server'];
}
...
function isWorkerRuntime(value: unknown): value is WorkerRuntimeId {
  return value === 'codex' || value === 'narada-agent-runtime-server';
}

function assertWorkerRuntime(value: unknown): asserts value is WorkerRuntimeId {
  if (value === 'deepseek-api') throw diagnosticError('worker_runtime_migrated_to_nars_provider', 'worker_runtime_migrated_to_nars_provider', { runtime: value, provider: 'deepseek-api', required_runtime: 'narada-agent-runtime-server', remediation: 'Use overrides.runtime="narada-agent-runtime-server" with constraints.provider="deepseek-api".' });
  ...
  return {
    ...
    nars_site_semantics: {
      ...
      canonical_provider_binding: {
        owner: 'canonical_invocation_plan',
        selector_transport: 'bound_site_environment',
        selector_key: 'NARADA_INTELLIGENCE_PROVIDER',
        selector_crosses_worker_boundary: false,
        credential_projection: 'selected_provider_aliases_only',
        generic_credential_keys_projected: false,
      },
    ...
    runtimes: {
      ...
      'narada-agent-runtime-server': {
        ...
        site_root_resolution: 'constraints.site_root when provided; otherwise nearest parent containing a Narada Site marker above cwd',
        accepted_site_environment_keys: ['NARADA_SITE_ROOT', 'NARADA_WORKSPACE_ROOT', 'NARADA_AGENT_ID', 'NARADA_CARRIER_SESSION_ID', 'NARADA_MCP_SCOPE', 'NARADA_INTELLIGENCE_PROVIDER', 'NARADA_INTELLIGENCE_ADAPTER_ID', 'NARADA_AI_API_KEY', 'NARADA_AI_BASE_URL', 'NARADA_AI_MODEL', 'NARADA_AI_THINKING', 'NARADA_MAX_TOOL_ROUNDS', 'NARADA_INTELLIGENCE_REGISTRY_DB', 'NARADA_INTELLIGENCE_TARGET_SITE', 'NARADA_INTELLIGENCE_USER_SITE', 'NARADA_INTELLIGENCE_HOST_SITE', 'NARADA_INTELLIGENCE_PRINCIPAL_ID', 'NARADA_INTELLIGENCE_PRINCIPAL_BINDING', 'CODEX_HOME', 'CODEX_CONFIG_DIR'],
        provider_runtime_binding_schema: 'narada.carrier.provider_runtime_binding.v1',
        provider_alias_posture: 'selected_provider_aliases_only',
        canonical_provider_binding: {
          owner: 'canonical_invocation_plan',
...
export function resolveSandbox(value: unknown, policy: WorkerPolicy, runtime?: WorkerRuntimeId): SandboxMode {
  const defaultSandbox = runtime === 'narada-agent-runtime-server'
